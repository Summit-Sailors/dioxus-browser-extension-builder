...
pub async fn extract(mode: ExtractionMode) -> Result<String, anyhow::Error> {
	...
	js_sys::Reflect::set(&query_obj, &JsValue::from_str("currentWindow"), &JsValue::from_bool(true))
		.map_err(|_| anyhow!("Failed to set currentWindow property"))?;
	...
	// the request payload
	let request = ExtractRequest { action: "extractContent".to_string(), mode };

	let request_js = JsValue::from_serde(&request).map_err(|_| anyhow!("Failed to serialize request"))?;
	...
	let send_promise = send_message_fn
	...
	let response_js = JsFuture::from(send_promise).await.map_err(|e| anyhow!("Failed to get response from content script: {:?}", e))?;

	let response: ExtractResponse = response_js.into_serde().map_err(|e| anyhow!("Failed to deserialize response: {:?}", e))?;
